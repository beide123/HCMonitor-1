...
//	strcat(filename, ".trace");
	...
	if((fp = open(filename,O_RDWR|O_CREAT|O_DIRECT)) < 0)
	{
	...
	*/	
	...
	close(ff);
	char *mem = malloc(128*1024*1024+4095);
	if(!mem){
		printf("malloc mem failed\n");
	}
	int size = 4096;
	int alg = 512 - 1;
	if(write(fp, ((unsigned int)memory_dev_addr+alg)&~alg, size)!=size){
		printf("errno is %d\n", errno);
	}	
	if(write(fp, ((unsigned int)mem+alg)&~alg, size)!=size){
		printf("errno is %d\n", errno);
	}	
	/*
